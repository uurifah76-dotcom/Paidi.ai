...
# 2. Inisialisasi Session State Asli Paidi.ai
...
# 3. CSS Kustom Asli Paidi.ai (Dark Mode & Bottom Nav)
st.markdown("""
    <style>
...
# 4. Header Logo & Studio Title Asli
st.markdown("""
...
# 5. Logika Navigasi Utama Berdasarkan Menu Bawah
if st.session_state.active_menu == "Beranda":
    
    # Tombol Tab Sub-Menu Atas (Pengganti segmented_control yang error)
        ...
        if st.session_state.credits <= 0:
            ...
            st.markdown("""
                <div style="background: linear-gradient(135deg, #0284c7, #2563eb); padding: 18px; border-radius: 16px; margin-bottom: 20px;">
                    <h4 style="color: white; margin: 0; font-size: 0.95rem;">🚀 STUDIO VIDEO OTOMATIS</h4>
                    <p style="color: #e0f2fe; font-size: 0.8rem; margin: 5px 0 0 0;">Ubah video panjang YouTube menjadi klip pendek viral dalam hitungan detik.</p>
                </div>
            """, unsafe_allow_html=True)
        ...
        if st.button("✨ Eksekusi Analisis Klip", use_container_width=True):
            if st.session_state.credits <= 0:
                ...
                with st.spinner("Paidi.ai sedang memproses video..."):
                    st.success("Klip berhasil dibuat berdasarkan pengaturan!")
            else:
                st.warning("Silakan masukkan tautan YouTube terlebih dahulu.")

        # Bagian Bawah: Legalitas & Founder (Untuk Kepercayaan Audiens)
