...
# 2. Inisialisasi Session State Utama & Konfigurasi Asli Paidi.ai
...
# State konfigurasi pengaturan studio
...
# Tangkap navigasi dari query parameter bawah
query_params = st.query_params
if "menu" in query_params:
    st.session_state.active_menu = query_params["menu"]

# 3. CSS Kustom Asli Paidi.ai (Dark Mode Mobile-First)
st.markdown("""
    <style>
...
if st.session_state.active_menu == "Beranda":
    ...
    # Tombol Tab Horizontal Asli Paidi.ai
    ...
    if st.session_state.active_tab == "Buat Klip":
        ...
        if st.session_state.credits <= 0:
            ...
            st.markdown("""
                <div style="background: linear-gradient(135deg, #0284c7, #2563eb); padding: 18px; border-radius: 16px; margin-bottom: 20px;">
                    <h4 style="color: white; margin: 0; font-size: 0.95rem;">🚀 Misi Utama Paidi.ai</h4>
                    <p style="color: #e0f2fe; font-size: 0.8rem; margin: 5px 0 0 0;">Mengubah video panjang YouTube menjadi klip viral otomatis untuk kreator Indonesia.</p>
                </div>
            """, unsafe_allow_html=True)

        # Form Input Link YouTube Utama
        ...
        # Indikator Status Konfigurasi Aktif
        st.markdown(f"""
            <div style="font-size: 0.75rem; color: #9ca3af; margin: 10px 0 15px 0;">
                🛠️ Konfigurasi Aktif: <span style="color: #38bdf8;">{st.session_state.cfg_durasi}</span> | <span style="color: #38bdf8;">{st.session_state.cfg_rasio}</span>
            </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("✨ Eksekusi Analisis Klip", use_container_width=True):
            if st.session_state.credits <= 0:
                ...
                with st.spinner("Paidi.ai sedang memproses video..."):
                    st.success("Klip berhasil dibuat sesuai pengaturan!")
            else:
                st.warning("Silakan masukkan tautan YouTube terlebih dahulu.")

        # ========================================================
        # INFORMASI LEGALITAS & FOUNDER (Pemanis Kepercayaan Audiens)
        # ========================================================
        ...
        st.markdown("<p style='color: #9ca3af; font-size: 0.8rem;'>Pilih parameter render klip video Anda di bawah ini:</p>", unsafe_allow_html=True)
        ...
        st.success("✨ Pengaturan berhasil disimpan otomatis.")
...
# 6. Render Navigasi Bawah Asli Paidi.ai
